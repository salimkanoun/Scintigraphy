...
public class TabTAC {
	...
	public JPanel getResultContent() {
		ModelSecondMethodHepaticDynamic modele = (ModelSecondMethodHepaticDynamic) ((TabOtherMethod) this.tab).getFenApplication().getFenApplication().getControleur().getModel();

		JPanel grid = new JPanel(new GridLayout(2, 2));

		SecondHepaticScintigraphy fenApplication = ((TabOtherMethod) this.tab).getFenApplication();

		BufferedImage capture = fenApplication.getFenApplication().getImagePlus().getBufferedImage();
		...
		chartDuodenom
				.setPreferredSize(new Dimension(parent.getWidth()/2,parent.getHeight()/2));
