...
public class Controleur_VG_Dynamique implements ActionListener {
	...
	private int reponseFinBDFAntre;//la reponse de l'utilisateur quand il est demande si sur la serie courant la region de l'antre est bruit de fond 

	private int reponseFinBDFIntestin;//la reponse de l'utilisateur quand il est demande si sur la serie courant la region de l'intestin est bruit de fond 
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (b == laVue.lesBoutons.get("Suivant")) {
			...
			case OEUFOUVERT:
				...
				if (reponse == 0) {
					...
					//si on reponds "No", on passe a l'etat CORRIGER, et on tenir compte que tous les oeufs ont la m¨ºme taille pour corriger les resultats
					etat = etat.next();
					index_Instru++;
					etat = etat.next();
					index_Instru++;
					...
					ActionEvent e = new ActionEvent(laVue.lesBoutons.get("Suivant"),ActionEvent.ACTION_PERFORMED,"Suivant");
			...
			case OEUFS:
				//si aucunne ROI est selecte, on remet la ROI precedente pour qu'on puisse le modifie 
				if (laVue.imp.getRoi()==null){
		...
		//ESTOMAC_ANT, INTESTIN_ANT, ESTOMAC_POS, INTESTIN_POS, CIR_ESTOMAC_ANT, CIR_INTESTIN_ANT, CIR_ESTOMAC_POS, CIR_INTESTIN_POS, OEUFOUVERT, ROIDEFAULT, OEUFES, CORRIGER,FIN;
		...
		if (b == laVue.lesBoutons.get("Precedent")){
			...
			case ESTOMAC_POS:
				...
				//si la reponse etait "YES" a ce etat, on remet la reponse a "no", pour qu'on puisse redemander l'utilisateur
	...
	private void addRoi(String nom) {
		...
		//en cas de ingestion, on update l'Overlay toujours, mais en cas de oeufs, on update l'Overlay juste la premire oeuf
		if(index_Oeuf==1){
		//on update l'Overlay
		laVue.overlay.clear();
		laVue.addOverlayGD();
	...
	// permet de revenir en arriere, le parametre i signifie le nombre de ROIs a revenir en arriere
	private void retour() {
		laVue.imp.killRoi();
		index_Roi--;
		laVue.leRoi.deselect();
	...
	private void getAntreFundus(String cote) {
		...
		if (WindowManager.getCurrentImage().getRoi()== null) {
			...
			//pour la deuxieme dimension de tableau coupsBrut, j est 0: image ANT, j est 1: image POS
			int j = 1;
			if (cote == "Ant") {
				j = 0;
			}
	...
	//permet de demander a l'utilisateur de preciser sur la serie courant si la region de l'antre est bruit de fond  
	...
	//permet de demander a l'utilisateur de preciser sur la serie courant si la region de l'intestin est bruit de fond  
