...
//					+ ", " + counts[2] + "))");
	...
	public void calculateResults() {
		if (this.fullBodyImages != 0) {
			...
			Double correctedKLAnt = this.data.get("Kidney L A")[0] - (meanBdfAntKidneyL * this.data.get(
					"Kidney L A")[2]);
			...
			Double correctedKRAnt = this.data.get("Kidney R A")[0] - (meanBdfAntKidneyR * this.data.get(
					"Kidney R A")[2]);
			Double correctedKRPost = this.data.get("Kidney R P")[0] - (meanBdfPostKidneyR * this.data.get(
					"Kidney R P")[2]);

			// Calculation of corrected Blader uptake
			Double correctedBladAnt = this.data.get("Bladder A")[0] - (meanBdfAntBladder * this.data.get(
					"Bladder A")[2]);
			Double correctedBladPost = this.data.get("Bladder P")[0] - (meanBdfPostBladder * this.data.get(
					"Bladder P")[2]);
...
//					System.out.println("Label : ");
					...
					if (s.startsWith("ContE")) {
						if (!contE.containsKey(number)) contE.put(number, new Double[2]);

						if (s.contains("A")) contE.get(number)[0] = this.data.get(s)[0];
						else if (s.contains("P")) contE.get(number)[1] = this.data.get(s)[0];

					} else if (s.startsWith("ContL")) {
						if (!contL.containsKey(number)) contL.put(number, new Double[2]);

						if (s.contains("A")) contL.get(number)[0] = this.data.get(s)[0];
						else if (s.contains("P")) contL.get(number)[1] = this.data.get(s)[0];
...
//				this.sumContL += Library_Quantif.moyGeom(contL.get(i)[0], contL.get(i)[1]);
	...
	public String toString() {
		...
		if (this.fullBodyImages != 0) {
			...
			if (this.deuxPrises) s += "WB early (5mn)," + Library_Quantif.round(this.totEarly, 2) + "\n";
