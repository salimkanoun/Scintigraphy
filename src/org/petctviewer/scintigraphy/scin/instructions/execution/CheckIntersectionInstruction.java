...
public class CheckIntersectionInstruction extends ExecutionInstruction {
	...
	/**
	 * @param intersection1    First instruction from which the intersection must be computed
	 * @param intersection2    Second instruction from which the intersection must be computed
	 * @param nameIntersection Name of the intersection (for display purposes)
	 */
	public CheckIntersectionInstruction(ControllerScin controller, Instruction intersection1,
	...
	/**
	 * Checks that there is an intersection between the previous ROI and the current one.
	 */
	private boolean checkIntersectionBetweenRois() {
		this.controller.getRoiManager().setSelectedIndexes(
				new int[]{this.previous_1.getRoiIndex(), this.previous_2.getRoiIndex()});
		...
		if (this.controller.getVue().getImagePlus().getRoi() == null) {
			JOptionPane.showMessageDialog(this.controller.getVue(), "Please adjust the " + previous_2.getRoiName() +
