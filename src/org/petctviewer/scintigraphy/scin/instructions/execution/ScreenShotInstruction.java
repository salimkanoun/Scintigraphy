...
public class ScreenShotInstruction extends ExecutionInstruction {
	 ...
	 * @param captureIndex  Index in the capture array where this instruction will
	 *                      store the new capture
	...
	/**
	 * The {@link #DEFAULT_CAPTURE_WIDTH} will be taken with this instantiation and
	...
	public void prepareAsNext() {
		...
		if (this.captures.size() > this.captureIndex) {
			...
			System.err.println("Error during capture:\nThe index is " + this.captureIndex
					+ " but the list is only at index " + this.captures.size()
